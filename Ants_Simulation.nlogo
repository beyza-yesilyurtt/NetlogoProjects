...
to setup
  ...
  set nest-x 2.5 + min-pxcor                      ;; set up nest and food locations
  set nest-y 0
  set food-x max-pxcor - 2.5
  set food-y 0
 ...
 create-leaders 1
  ...
  create-followers (num-ants - 1)
    [ set color yellow                             ;; middle ants are yellow
      set headıng 90 ]                             ;; and start with a fixed heading
  ask turtles
  ...
  if all? turtles [xcor >= food-x]
   ...
   ask leaders                                      ;; the leader ant wiggles and moves
     [ wiggle leader-wiggle-angle
       correct-path
       if (xcor > (food-x - 1.25 ))                    ;; leader heads straight for food, if it is close
