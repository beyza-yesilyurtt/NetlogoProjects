...
dırected-lınk-breed [connectors connector]
...
to move-flagella
  ...
  set headıng [ headıng ] of my-bacteria + new-swing
end

to check-caught
  if not mouse-down? or not mouse-inside? [ stop ]
  let prey [bacteria in-radius (sıze / 2)] of one-of predators
...
to visualize-bacteria
  ...
  ask flagella [
    set hıdden? member? visualize-variation ["as color only" "# flagella as label" "none"]
  ]
end
